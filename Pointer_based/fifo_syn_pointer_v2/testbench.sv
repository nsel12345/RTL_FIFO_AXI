//Addded 'random constraints' in transaction and modified randomize statement in generator. we can test backpressure here.
`include "fifo_if.sv"
...
class transaction #(parameter DATA_WIDTH = 32);
  ...
  constraint c_wr{wr_en dist {1:=80, 0:=20};} //Write 80% of the time
  constraint c_rd{rd_en dist {0:=80, 1:=20};} //Read 20% of the time
