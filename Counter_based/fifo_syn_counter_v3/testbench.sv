//implemented try_get in driver under task run.
// did this to fix: in prev version v2., driver  which is on forever loop gets stuck on the last value wr_en, if wr_en = 1 then the last din value gets written into until fifo is full
...
module tb_fifo_syn_counter;
 ...
 logic rst;
  ...
  fifo_syn_counter #(.DATA_WIDTH(DATA_WIDTH), .DEPTH(DEPTH)) dut ( .dout(pif.dout), .full(pif.full), .empty(pif.empty), .clk(clk), .rst(rst), .wr_en(pif.wr_en), .rd_en(pif.rd_en), .din(pif.din));
  ...
  initial 
    begin
      ...
      $dumpfile("dump.vcd");
      $dumpvars(0, tb_fifo_syn_counter);
  ...
  #650; // an approx delay to allow driver to finish emptying the mailbox
