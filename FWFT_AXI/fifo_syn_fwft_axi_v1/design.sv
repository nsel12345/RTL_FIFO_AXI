...
//Description: Here in this design file the goal is to implement axi stream protocol
//using fwft wrapper
...
module fifo_syn_pntr_axi_fwft #(parameter DATA_WIDTH = 32, DEPTH = 16)
  ...
  //In axi wr_en = 1 if s_tvalid = 1 and s_tready = 1
  assign native_wr_en = s_tvalid && s_tready;
  
  // rd_en = 1 if data is available (!native_empty) and (fwft_valid is empty or  receiver is ready)
  assign native_rd_en = !native_empty && (!fwft_valid || m_tready);
  
  logic fwft_valid; //a signal that shows if data is available for output transaction
  
  // read logic, main part:
  assign m_tvalid = fwft_valid;
  assign m_tdata = native_dout;
  
  //A timed block to control and sync 'fwft_valid'
